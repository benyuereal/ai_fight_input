...
# 账号
accounts = ['18850575164','']

# 密码
passwords = ['Welcome2sohu!','']

driverEdge = [True, None]
    ...
    def get_page(self, reset_to_homepage=0):
        ...
        try:
            ...
            if self.check_verification_code():
                ...
                # 单程
                # 隐藏遮挡元素
                obstacle_element = self.driver.find_element(By.CLASS_NAME, "lsn_menu_son_nav_HoNwa")
                self.driver.execute_script("arguments[0].style.display = 'none';", obstacle_element)
...
def search(begin_date, end_date, crawal_citys):
    if driverEdge[0]:
        driver = init_driver()
        print("edege 完成初始化")
        driverEdge[0] = False
    ...
    for city in citys:
        ...
        for flight_date in flight_dates:
            ...
            if ('http' not in Flight_DataFetcher.driver.current_url):
